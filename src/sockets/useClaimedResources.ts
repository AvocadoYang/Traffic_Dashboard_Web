...
export const useClaimedRoads = () => {
  const [claimedRoads, setClaimedRoads] = useState<Map<string, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    const sub = claimedResource$.subscribe(({ roads }) => {
      setClaimedRoads(
        new Map(roads.map(({ roadId, amrId }) => [roadId, amrId])),
      );
    });
...
export const useClaimedLocations = () => {
  const [claimedLocations, setClaimedLocations] = useState<Map<string, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    const sub = claimedResource$.subscribe(({ locations }) => {
      setClaimedLocations(
        new Map(locations.map(({ locationId, amrId }) => [locationId, amrId])),
      );
    });
